...
namespace nether::rendering
{
	CommandQueue::CommandQueue(ID3D12Device5* const device, const D3D12_COMMAND_LIST_TYPE commandListType, const std::wstring_view commandQueueName)
	{
		...
		SetName(mCommandQueue.Get(), commandQueueName.data());
		...
		for (const uint32_t frameIndex : std::views::iota(0u, NUMBER_OF_FRAMES))
		{
			...
			SetName(mCommandAllocators[frameIndex].Get(), commandQueueName.data());
		}

		// Create synchronization primitives.
		utils::ThrowIfFailed(device->CreateFence(0u, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&mFence)));
		SetName(mFence.Get(), L"Direct Command Queue Fence");
	...
	void CommandQueue::SetFrameFenceValues(const uint32_t frameIndex)
	{
		for (uint32_t index : std::views::iota(0u, frameIndex))
		{
			mFrameFenceValues[index] = mFrameFenceValues[frameIndex];
		}
	}
