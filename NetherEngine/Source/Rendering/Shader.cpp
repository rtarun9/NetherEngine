...
namespace nether::rendering
{
	...
	Shader ShaderCompiler::CompilerShader(const ShaderType& shaderType, const std::wstring_view shaderPath, const std::wstring_view baseShaderDirectory)
	{
		...
		for (uint32_t i : std::views::iota(0u, shaderDesc.BoundResources))
		{
			...
			case D3D_SIT_CBUFFER:
			{
				...
				const std::wstring bindingName = utils::StringToWString(shaderInputBindDesc.Name);
				shader.shaderReflection.rootParameterBindingMap[bindingName] = shader.shaderReflection.rootParameters.size();
			...
			case D3D_SIT_TEXTURE:
			{
				srvDescriptorCount++;
				registerSpace = shaderInputBindDesc.Space;
				srvShaderResourceBindingDescName = utils::StringToWString(shaderInputBindDesc.Name);
			}break;
			}
		}
		...
		if (srvDescriptorCount > 0)
		{
			...
			const std::wstring bindingName = srvShaderResourceBindingDescName;
			shader.shaderReflection.rootParameterBindingMap[bindingName] = shader.shaderReflection.rootParameters.size();
	...
	Microsoft::WRL::ComPtr<IDxcBlob> ShaderCompiler::CompilerShader(const ShaderType& shaderType, const std::wstring_view shaderPath, const std::wstring_view baseShaderDirectory, ShaderReflection& shaderReflectionOutput)
	{
		// Create utils, compiler and include handler if they are not yet created.
		// If mUtils is nullptr, it means the other two members are also null.
		if (!mUtils)
		...
		// Get the input element desc's from the shader reflection desc. Done only for vertex buffers here.
		if (shaderType == ShaderType::Vertex)
		{
			shaderReflectionOutput.inputElementDescs.reserve(shaderDesc.InputParameters);
			...
			for (uint32_t i : std::views::iota(0u, shaderDesc.InputParameters))
			{
				...
				shaderReflectionOutput.inputElementDescs.emplace_back(InputElementDesc{ .semanticName = inputParameterDesc.SemanticName, .inputElementDesc = inputElementDesc });
			}
		}
	
		// Get root parameters from shader reflection data (constant buffers views will be exclusively used for now).
		shaderReflectionOutput.rootParameters.reserve(shaderDesc.BoundResources);
		...
		for (uint32_t i : std::views::iota(0u, shaderDesc.BoundResources))
		{
			...
			case D3D_SIT_CBUFFER:
			{
				...
				const std::wstring bindingName = utils::StringToWString(shaderInputBindDesc.Name);
				shaderReflectionOutput.rootParameterBindingMap[bindingName] = shaderReflectionOutput.rootParameters.size();
				shaderReflectionOutput.rootParameters.emplace_back(rootParameter);
			}break;
		...
		if (srvDescriptorCount > 0)
		{
			...
			shaderReflectionOutput.descriptorRanges.push_back(srvDescriptorRange);
			...
			const D3D12_ROOT_PARAMETER1 srvDescriptorTable
			{
				...
				.DescriptorTable
				{
					.NumDescriptorRanges = static_cast<uint32_t>(shaderReflectionOutput.descriptorRanges.size()),
					.pDescriptorRanges = shaderReflectionOutput.descriptorRanges.data(),
				},
				.ShaderVisibility = shaderVisibility,
			};
			...
			const std::wstring bindingName = srvShaderResourceBindingDescName;
			shaderReflectionOutput.rootParameterBindingMap[bindingName] = shaderReflectionOutput.rootParameters.size();
			shaderReflectionOutput.rootParameters.push_back(srvDescriptorTable);
		}
